...
export default class World {
  ...
  constructor(width, height) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let column = 0; column < columns; column++)
        this.#chunks.set(`${row},${column}`, new ChunkClass(row << 4, column << 4, 16, 16));
      this.#chunks.set(`${row},${columns}`, new ChunkClass(row << 4, columns << 4, 16, lastRow));
    }
    for (let column = 0; column < columns; column++) this.#chunks.set(`${rows},${column}`, new ChunkClass(rows << 4, column << 4, lastColumn, 16));
    this.#chunks.set(`${rows},${columns}`, new ChunkClass(rows << 4, columns << 4, lastColumn, lastRow));
  }
  ...
  setBlock(x, y, layer, block) {
    if ((block instanceof BlockClass) && (x >= 0) && (x < this.width)
      && (y >= 0) && (y < this.height)) {
      this.#chunks.get(`${x >> 4},${y >> 4}`).setBlock(x, y, layer, block);
    }
  }
  ...
  draw(playerViewX, playerViewY, playerViewW, playerViewH, ctx) {
    ...
    const endX = Math.min(this.width >> 4, playerViewX + (playerViewW >> 4) >> 4)
    const endY = Math.min(this.height >> 4, playerViewY + (playerViewH >> 4) >> 4)

    for (let x = startX; x < endX + 1; x++) {
      for (let y = startY; y < endY + 1; y++) {
        this.#chunks.get(`${x},${y}`).drawChunk(((x << 4) - playerViewX) << 4, ((y << 4) - playerViewY) << 4, ctx)
      }
    }
  }
  getChunk(x, y) {
